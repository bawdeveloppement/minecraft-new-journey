...
import net.minecraft.tileentity.TileEntityDoor;
...
public class BlockDoor extends Block implements ITileEntityProvider{
   ...
   public static final PropertyBool HAS_SCHEME = PropertyBool.create("locked");
   ...
   {
	   if (this.getDefaultState().getValue(HALF) == BlockDoor.EnumDoorHalf.LOWER) {
		   return true;		   
	   } else {
		   return false;
	   }
   ...
   public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
      if(this.blockMaterial == Material.iron || worldIn.isRemote) {
         return true;
      } else {
         ...
         if(iblockstate.getBlock() != this) {
            return false;
        	 ...
        	 if (playerIn.getCurrentEquippedItem() != null) {
        		 ItemStack currentStack = playerIn.getCurrentEquippedItem();
        		 NBTTagCompound nbt = currentStack.getTagCompound();
        		 TileEntityDoor doorEntity = (TileEntityDoor) worldIn.getTileEntity(blockpos);
        		 if (doorEntity != null) {
        			 if (nbt != null && nbt.getString("key_scheme_id") != null) {
        				 if (nbt.getString("key_scheme_id").toString() == doorEntity.getKeySchemeId().toString()) {
        					 doorEntity.LockOrUnlock(currentStack);
        				 ...
        				 doorEntity.LockOrUnlock(currentStack);
            			 // worldIn.playAuxSFXAtEntity(playerIn, ((Boolean)state.getValue(OPEN)).booleanValue()?1003:1006, pos, 0);
            			 System.out.println("Mauvaise clé mais on continue");
            			 return true;
        			 }
        		 } else {
        			 System.out.println("The door entity was not found");
        			 return false;
        		 }
        	 } else {
        		 if (worldIn.getTileEntity(blockpos) != null) {
        			 ...
        			 System.out.println("The door haven't entity");
        			 return false;
        		 }
        	 }
         }
      ...
      if(state.getValue(HALF) == BlockDoor.EnumDoorHalf.UPPER) {
         ...
         if(flag1) {
            ...
            if((flag || neighborBlock.canProvidePower()) && neighborBlock != this && flag != ((Boolean)iblockstate1.getValue(POWERED)).booleanValue()) {
               ...
               TileEntityDoor doorTileEntity = (TileEntityDoor) worldIn.getTileEntity(pos);
               if(flag != ((Boolean)state.getValue(OPEN)).booleanValue() && doorTileEntity != null && !doorTileEntity.isLocked()) {
