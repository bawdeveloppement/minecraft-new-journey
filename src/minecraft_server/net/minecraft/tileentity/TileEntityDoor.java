...
public class TileEntityDoor extends TileEntity {
	private UUID key_scheme_id = UUID.randomUUID();
	...
	public void LockOrUnlock(ItemStack keyStack) {
		if (keyStack.getItem() == Items.key) {
			if (keyStack.getTagCompound() != null) {
				if (keyStack.getTagCompound().getString("key_scheme_id") != null) {
					if (keyStack.getTagCompound().getString("key_scheme_id").equals(this.key_scheme_id)) {
